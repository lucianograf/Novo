...
//variaveis DECANTER - TSCB18 Luis França 
Private cMensIcmSt  := ''
// 26/11/2020 - Marcelo Alberto Lauschner - Decanter - Informar e-mail de transportadora de redespacho no xml - 
Private cMailRedesp	:= ''
// 10/06/2021 - Marcel oAlberto Lauschner - Decanter - Data de Saída/Entrega inforamda no pedido de venda.
...
Else	
	...
	cString += NfeTag('<dSaiEnt>',Iif(lDSaiEnt .And. Empty(dDatSaiEnt), "", Iif(!Empty(dDatSaiEnt),ConvType(dDatSaiEnt), ConvType(aNota[03])) )) 
	//If !lDSaiEnt .And. !Empty(aNota[06]) 
	// Customização Decanter - 10/06/2021 - Só preenche a hora de saída no Default, pois se a data de saída veio do pedido de venda, não assume hora, pois não há a informação. 
	If !lDSaiEnt .And. !Empty(aNota[06]) .And. Empty(dDatSaiEnt) // 
		if len(aNota[06]) > 5
