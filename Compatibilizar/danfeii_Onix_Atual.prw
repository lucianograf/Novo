...
//Compatibilizado em 17/06/2026 - Luciano - Lauscher Consulting
...
User Function PrtNfeSef(cIdEnt, cVal1		, cVal2		, oDanfe,;
	...
	If nTipo <> 1
		...
		Else
			...
			//Não altera as perguntas quando for chamado pela API de consulta
			// Customização Onix/Frimazo -
			If lVerPerg .And. !FwIsInCallStack('U_CONSULTAPDF')
				//Não abre a pergunta se for chamado pelo BFFATA16
				if !lJob .And. !FwIsInCallStack("U_BFFATA16")
...
// Customização Frimazo - impressão de relatório de itens da nota 
	If FwCodEmp() == '05' .And. MV_PAR04 == 2
		DbSelectArea("SD2")
		DbSetOrder(3) // D2_FILIAL+D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_COD+D2_ITEM
		DbSeek(xFilial("SD2")+MV_PAR01+MV_PAR03)
		U_FZPCPR02(.T.,SD2->D2_PEDIDO)
	EndIf
...
User Function DANFEProc(	oDanfe	, lEnd		, cIdEnt	, cVal1,;
	...
	Local iP // Customização Onix/Frimazo - Variável de Loop
	Default lEnd		:= .F.
	Default lIsLoja		:= .F.
	Default nTipo		:= 0
						...
						If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
							...
							Else
								...
								If MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
									// Customização Onix/Frimazo
									If RetCodUsr() $ GetNewPar("GM_USRPTDF",__cUserId)
...
// Customização Onix/Frimazo 
	Local	cProdAgl	:= ""
	Local	cProdLote	:= ""
	Local	lAgltPrdLt	:= cEmpAnt+cFilAnt $ "0501#0503"
	Local	aItensAglt	:= {}
	Local	nLastLin	:= 0
	Local	cMsgAuxDf	:= ""
// Fim Customização 
...
// Customização Onix/Frimazo 
	If(MV_PAR04==2)
		...
		If !Empty(SD2->D2_PEDIDO) .And. __cUserId $ GetNewPar("GM_USRPTDF",__cUserId)
			...
			// 17/12/2025 - Verifica se cliente tem Desconto Acordo Comercial e não imprime Boleto Obrigatório ( banco 400 )
			If (SA1->(FieldPos("A1_XDESC")) > 0 .And. SA1->(FieldPos("A1_BCO1")) > 0 .And. SA1->A1_XDESC > 0 .And. !SA1->A1_BCO1 $ "400")
				Dbselectarea("SC5")
			...
			Endif
			If (SC5->C5_BANCO $ ("887#988#993#985#987#900#991#992" +  GetNewPar('BF_PORSBOL', '900/987/985/007/008/006/'))) // Chamado 592 - Atribuir portador automático sem impressão de boleto

				DbSelectArea("SA6")
				Dbsetorder(1)
				DbSeek(xfilial("SA6")+SC5->C5_BANCO)

				// 23/01/2019 Marcelo Lauschner
				// Gera mensagem de Observação no DANFE
				If FwCodEmp() == "05" .And. SC5->C5_BANCO $ "991#992"
					cMsgAuxDf	+= "SR.MOTORISTA FAVOR SE ATENTAR PARA A COBRANÇA ***" + Alltrim(SA6->A6_NOME) + "*** "
				Endif
				...
				While !Eof() .and. xFilial("SE1")  == SE1->E1_FILIAL .And. SE1->E1_PREFIXO == SF2->F2_PREFIXO .And.;
					...
					// 16/12/2025  - Gera abatimento de acordo comercial já na impressão do Danfe quando for um portador que não terá impressão de boletos
					If FwCodEmp() == "05" .And. SE1->E1_DESCFIN == 0  .And. SA1->A1_XDESC > 0
						nA1_XDESC	:= SA1->A1_XDESC
						nDescFin 	:= (SE1->E1_VALOR*nA1_XDESC)/100

						aAreaSE1	:= SE1->(GetArea())
						// Efetua a baixa parcial do título
						//A função sfExec070 está no fim do código
						sfExec070(SE1->E1_PREFIXO,SE1->E1_NUM,SE1->E1_PARCELA,SE1->E1_TIPO,nDescFin,nA1_XDESC)
						RestArea(aAreaSE1)

						Dbselectarea("SE1")
						RecLock("SE1",.F.)
						SE1->E1_DESCFIN := nA1_XDESC //aDescCli[y,2]
						MsUnLock()
						nDescFin := 0 // Zero o valor do Desconto para que nÃ£o seja impresso no Boleto nos campos de desconto
					Endif
...
//oDanfe:Say(017, 510, "N. "+StrZero(Val(oIdent:_NNf:Text),9), oFont08:oFont)
		oDanfe:Say(017, 510, "N. "+StrZero(Val(oIdent:_NNf:Text),9), oFont18N:oFont) //Customizado, alterado oFont de 08N para 18N
...
// Customização Onix/Frimazo
		If oIdent:_TpNf:Text == "1"
			...
			oDanfe:Say(036, 102, "Emissão: "+ Iif(oNF:_INFNFE:_VERSAO:TEXT >= "3.10",ConvDate(oIdent:_DHEmi:TEXT),ConvDate(oIdent:_DEmi:TEXT)) + " Destinatário: "+ NoChar(oDestino:_XNome:TEXT,lConverte)+ " Valor Total R$ " + Alltrim(aTotais[11]), oFont07:oFont)
...
// Customização Frimazo 
		If FwCodEmp() == "05"
			cStrAux		:=	AllTrim(NoChar("Email NFe: nfe@frimazo.com.br"))
		Endif
// Final Customização 

//nForTo		:=	Len(cStrAux)/40
		nForTo		:=	Len(cStrAux)/45 // Customização ONix/Frimazo - Ajuste de quantidade de caracteres
		...
		If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
			...
			EndIf
			//nForTo		:=	Len(cStrAux)/40
			nForTo		:=	Len(cStrAux)/45 // Customização Onix/Frimazo - Ajuste de caracteres por linha
			nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
			For nX := 1 To nForTo
				oDanfe:Say(nLinCalc,096,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*40)+1),40),oFont08N:oFont)
				nLinCalc+=10
...
Static Function setQuery(cTabela, cCampos, lOrdena)
	...
	elseif cTabela == 'SF3'
		...
		//Customização FRimazo/Onix
		If IsInCallStack("U_BFFATA16") .And. Type("aRecSF2") == "A" .And. Len(aRecSF2) > 0
			oQryExec:SetIn(nCont++, aRecSF2) // Numero de notas selecionadas
		Endif
		// Fim da customização
	endif
...
//A função sfExec070 é chamada no meio do código atualmente na linha 1.746 
Efeuta baixa parcial referente Crédito de Acordo Comercial 
@type function
@version  
@author Lauschner Consulting - Marcelo Alberto Lauschner
@since 11/01/2026
@param cE1PREFIXO, character, param_description
@param cE1NUM, character, param_description
@param cE1PARCELA, character, param_description
@param cE1TIPO, character, param_description
@param nInDescFin, numeric, param_description
@param nA1DESC, numeric, param_description
@param cHistBx, character, param_description
@return variant, return_description
/*/
Static Function sfExec070(cE1PREFIXO,cE1NUM,cE1PARCELA,cE1TIPO,nInDescFin,nA1DESC,cHistBx)

	Local 	aBaixa 		:= {}
	Local	aAreaOld	:= GetArea()
	Default cHistBx 	:= "DESC.AUT." + cValToChar(nA1DESC)+"%"
	// Grava as perguntas fixas para o usuário
	U_gravasx1("FIN070","01",2)	// Mostra Lançamento contábil 1=Sim/2=Não
	U_gravasx1("FIN070","02",2)	// Aglutina Lançamentos 1=Sim/2=Não
	U_gravasx1("FIN070","04",1)	// Contabiliza Online 1=Sim/2=Não
