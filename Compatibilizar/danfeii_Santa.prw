...
/*Compatibilizado em 16/06/2026 - Luciano - Lauschner Consulting*/
...
// Customizado SantaAgro - Ajuste da variável maxitempc
							...
							If MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
								...
								IF SF2->(FieldPos("F2_FILDEST"))> 0 .And. SF2->(FieldPos("F2_FORDES"))> 0 .And.SF2->(FieldPos("F2_LOJADES"))> 0 .And.SF2->(FieldPos("F2_FORMDES"))> 0 .And. !EMPTY (SF2->F2_FORDES)
							       SF1->(dbSetOrder(1))
							    	...
							    	Endif
							    EndiF
								// Customização Santa Agro - Grava log de impressão do Danfe 
								U_GMCFGM01("IN",,"Impressão da Danfe "+SF2->F2_SERIE+"/"+SF2->F2_DOC ,FunName())
...
// Customização Santa Agro - impressão dados adcionais no canhoto 
