...
User Function VTEX002()
	...
	Else
		...
		oGrid		:=	FWGridProcess():New(   "VTEX002",  "Buscar Clientes e Pedidos do vtex", cDescDet, {|lEnd| U_VTEX002P(@lEnd)}, "")
		oGrid:SetMeters(2)
...
User Function VTEX002P(lEnd)
	...
	Private cTranspEcm		:= GETMV("MA_TRANECM"  	,,"378")	//"378") //Transportadora
	...
	Begin Sequence
		// beginSQL Alias "VTEXTMP"
		// 	SELECT PED)ID FROM TEX010 WHERE XPTO //BUSCAR DA TABELA DE FEED
        // EndSQL
		...
		While !SB1TMP->(EOF())
            ...
            oRestClient:SetPath(cPath+VTEXTMP->PED_ID)
            ...
            if oRestClient:Get(aHeadOut)
                ...
                If FWJsonDeserialize(sPostRet,@oObj)
                    If SubStr(oRestClient:GetLastError(),1,3) == '200' .and. ValType(oObj) == "O"
                        //cProdID := oObj:Id
                        //TODO - GRAVAR NA TABELA TEMPORARIA
                    Else
        ...
        //BUSCAR PEDIDOS GRAVADOS NA TEMP LOCAL
        // beginSQL Alias "VTEXPED"
		// 	SELECT PED)ID FROM TEX010 WHERE XPTO //BUSCAR DA TABELA DE FEED
        // EndSQL

        VTEXPED->(dbGotop())
        While !VTEXPED->(EOF())
			cPedId		:= VTEXPED->PEDIDO_ID
			cCliId		:= VTEXPED->CLIENTE
			...
			If Len(aDadCli) == 0
				//FWLogMsg("ERROR","LAST",'1',"EXCLUIR_PRODUTO",,"vtex","Nenhum produto localizado para atualizar ou incluir")
				oActLog:Inf("VTEX002","Pedido: "+cPedId+ " Cliente: "+cCliId+ " - Nenhum dado do cliente encontrado")
				lRetDadCli := .F.
				//Break
			Else
				lRetDadCli := .T.
			...
			If Len(aDadEndCli) == 0
				//FWLogMsg("ERROR","LAST",'1',"EXCLUIR_PRODUTO",,"vtex","Nenhum produto localizado para atualizar ou incluir")
				oActLog:Err("VTEX002","Pedido: "+cPedId+ " Cliente: "+cCliId+ " - Nenhum dado de endereço do cliente encontrado")
				lRetEndCli := .F.
				//Break
			Else
				lRetEndCli := .T.
			...
			If Len(aCliDad) == 0
				//FWLogMsg("ERROR","LAST",'1',"EXCLUIR_PRODUTO",,"vtex","Nenhum produto localizado para atualizar ou incluir")
				oActLog:Err("VTEX002","Pedido: "+cPedId+ " Cliente: "+cCliId+ " - Não foi possível incluir/alterar o cliente.")
				lRetCliDad	:=	.F.
			ElseIf aCliDad[1]
				oActLog:Inf("VTEX002","Pedido: "+cPedId+ " Cliente: "+cCliId+ " - Cliente incluído/alterado com sucesso. Código: "+aCliDad[2]+" Loja: "+aCliDad[3])
				lRetCliDad	:=	.T.
			Else
				oActLog:Inf("VTEX002","Pedido: "+cPedId+ " Cliente: "+cCliId+ " - Cliente sem alterações, já estava incluído na base. Código: "+aCliDad[2]+" Loja: "+aCliDad[3])
				lRetCliDad	:=	.T.
			EndIf
			...
			If Len(aPedDad) == 0
				oActLog:Err("VTEX002","Pedido: "+cPedId+ " Cliente: "+cCliId+ " - Nenhuma informação do pedido encontrado")
				lRetPedDad	:=	.F.
				//Break
			Else
				lRetPedDad	:=	.T.
			...
			If !lCadPed
				oActLog:Err("VTEX002","Pedido: "+cPedId+ " Cliente: "+cCliId+ " - Não foi possível incluir/alterar o pedido")
			Else
				oActLog:Inf("VTEX002","Pedido: "+cPedId+ " Cliente: "+cCliId+ " - Pedido incluído com sucesso.")
			EndIf
...
Função responsavel alimentar o Protheus com os dados dos clientes no vtex.
...
Static Function DECACADCLI(aDadCli,aDadEndCli)
	...
	Begin Sequence
		...
		nPosCId		:= Ascan( aDadCli,{ |X| UPPER( AllTrim(X[1]) )=="_CUSTOMER_ID" } )	//Cod vtex (ID)
		...
		FOR nX := 1 TO Len(aDadEndCli)
			...
			If aDadEndClX[nPosEPri][2]:TEXT == "true"
				nPosEId		:= Ascan( aDadEndClX,{ |X| UPPER( AllTrim(X[1]) )=="_CUSTOMER_ADDRESS_ID" } )//Cod vtex (ID)
...
Função responsável por alimentar o Protheus com os dados dos pedidos do vtex.
...
Static Function DECACADPED(cPedId, aPedDad, aCliDad)
	...
	Begin Sequence
		...
		Else
			cMsgError	:=	 "Falha ao buscar a data de emissão do pedido do vtex. Posição: "+cValToChar(nPosDTEmi)
			FWLogMsg("ERROR","LAST",'1',"DECACADPED",,"vtex",cMsgError)
			oActLog:Err(cMsgError)
			MsgStop(cMsgError,"Erro - "+ProcName()+"/"+cValToChar(ProcLine()))
			Break
		...
		If nPosStatusMag <> 0
			...
			ElseIf cStatMgtWS == "canceled"
				cStatMgt := "6"
			Else
				//cMsgError	:=	 "Falha ao obter status do vtex. Posição: "+cValToChar(nPosMoeda)+" Moeda: "+cMoedaWS
				//FWLogMsg("ERROR","LAST",'1',"DECACADPED",,"vtex",cMsgError)
				//oActLog:Err(cMsgError)
				//MsgStop(cMsgError,"Erro - "+ProcName()+"/"+cValToChar(ProcLine()))
				//Break
		...
		If nPosObsEnt <> 0
			...
			If nPosObsEst <> 0
				...
				Else
					cMsgError	:=	 "Falha ao buscar o estado do endereço de entrega do pedido no array de estados. Estado vtex: "+cValToChar(nPosObsUFX)
					FWLogMsg("ERROR","LAST",'1',"DECACADPED",,"vtex",cMsgError)
					MsgAlert(cMsgError,Procname())
					Break
				EndIf
		...
		If SA1->A1_MSBLQL == "1"
			cMsgError	:=	 "Cliente ainda não revisado/liberado, cadastro está bloqueado no Protheus. Pedido "+cPedId+" do vtex não será importado."+chr(13)+chr(10)+"Cliente: "+cCodCli+" Loja: "+cLojCli
			FWLogMsg("INFO","LAST",'1',"DECACADPED",,"vtex",cMsgError)
			oActLog:Err(cMsgError)
		...
		If Empty(cCodCondP)
			cMsgError	:=	 "Código da condição de pagamento não localizado no DE/PARA. Código vtex:"+cCodPagMGT+" Conteúdo parâmetro MA_CONDPAG: "+cCondPag
			FWLogMsg("INFO","LAST",'1',"DECACADPED",,"vtex",cMsgError)
			oActLog:Err(cMsgError)
			//MsgAlert(cMsgError,"Info - "+ProcName()+"/"+cValToChar(ProcLine()))
			lRet := .F.
		...
		If !(cAliasSC5)->(Eof())
			...
			If DbSeek(xFilial("SC5")+(cAliasSC5)->C5_NUM)
				If C5_ZSTATMG	<> cStatMgt
					...
					U_WFGERAL("grupo.vtex@decanter.com.br"/*cEmail*/,"Alteração Status Pedido vtex '"+cPedId+"' "/*cTitulo*/,cMsgWf/*cTexto*/,"VTEX002"/*cRotina*/,/*cAnexo*/)
					RecLock("SC5",.F.)
...
//							MaTesInt(nEntSai,cTpOper,cClieFor,cLoja,cTipoCF,cProduto,cCampo,cTipoCli) --> cTesRet
		...
		If nValTotFun <> nVlrTot
			//cMsgError	:=	 "Valor total do pedido do vtex: "+cValToChar(nVlrTot)+" é diferente do valor total do pedido calculado: "+cValToChar(nValTotFun)
			//FWLogMsg("ERROR","LAST",'1',"DECACADPED",,"vtex",cMsgError)
			//oActLog:Err(cMsgError)
			//MsgStop(cMsgError,"Erro - "+ProcName()+"/"+cValToChar(ProcLine()))
			//Break
...
Static Function DECARESERV(aItens)
	...
	Begin Sequence
		...
		oSC0M:SetValue("C0_SOLICIT"	, "vtex"		)
		SB2->(dbSetOrder(1))
