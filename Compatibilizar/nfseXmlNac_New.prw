...
Private aImovel		 := &(SuperGetMV("MV_XMLIMOV", ,"{,,,,,,,,,,}"))
Private aImoveis     := {}
...
Private cNumCIBOb  := "" // Numero CIB para Obra
Private cInscImob  := "" // Inscrição imobiliária fiscal 
Private cNumCIBIm  := "" // Numero CIB para Imovel
Private cLogradIm  := "" //Logradouro para Imovel
Private cNumeroIm  := "" // Numero para Imovel
Private cCompleIm  := "" //Complemento para Imovel
Private cBairroIm  := "" // Bairro para Imovel
Private cCepIm     := "" // Cep para Imovel
Private cCodMunIm  := "" // Cod do Municipio para Imovel
Private cNomMunIm	 := "" // Nome do municipio para Imovel
Private cUfIm		 := "" // UF para Imovel
Private cCodPaisIm := "" // Codigo do Pais para Imovel
...
if !Empty(aNota)
    if Len(aProd) > 0
        ...
        If ( Date() >= dNfseNacIBS) 
            cString	+= IbsCbs( aDest, cCodMun, aNota, cClieFor, cLoja, aEntrega, aRetSF3, aProd, aImoveis )
        EndIf
        cString += '</infDPS>'
        cString += '</DPS>'
    EndIf
...
If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
    ...
    If !SF2->F2_TIPO $ "DB" 
        ...
        aadd(aDest,Alltrim(SF2->F2_TIPOCLI)) //30
        ...
        Else
            If lJescTur .And. FindFunction("JurTitPFS") // Integração com módulo SIGAPFS //12.1.2510
                cTitPFS  := JurTitPFS(SF2->F2_DOC, SF2->F2_SERIE, @cNatPFS, SF2->F2_FILIAL)
                cTitulo  := cTitPFS
                cNatBusc := cNatPFS
            Else
                cNatBusc := NatPCC ( aDest , cNatPCC )
    ...
    EndIf
    If ValType(aObra) <> "U"
        cLogradOb  := AllTrim(If(!Empty(aObra[01]) .And. SC5->(FieldPos(aObra[01])) > 0 , SC5->&(aObra[01]),"")) //Logradouro para Obra
        cCompleOb  := AllTrim(If(!Empty(aObra[02]) .And. SC5->(FieldPos(aObra[02])) > 0 , SC5->&(aObra[02]),"")) //Complemento para obra
        cNumeroOb  := AllTrim(If(!Empty(aObra[03]) .And. SC5->(FieldPos(aObra[03])) > 0 , SC5->&(aObra[03]),"")) // Numero para Obra
        cBairroOb  := AllTrim(If(!Empty(aObra[04]) .And. SC5->(FieldPos(aObra[04])) > 0 , SC5->&(aObra[04]),"")) // Bairro para Obra
        cCepOb     := AllTrim(If(!Empty(aObra[05]) .And. SC5->(FieldPos(aObra[05])) > 0 , SC5->&(aObra[05]),"")) // Cep para Obra
        cCodMunob  := AllTrim(If(!Empty(aObra[06]) .And. SC5->(FieldPos(aObra[06])) > 0 , SC5->&(aObra[06]),"")) // Cod do Municipio para Obra
        cNomMunOb  := AllTrim(If(!Empty(aObra[07]) .And. SC5->(FieldPos(aObra[07])) > 0 , SC5->&(aObra[07]),"")) // Nome do municipio para Obra
        cUfOb 	   := AllTrim(If(!Empty(aObra[08]) .And. SC5->(FieldPos(aObra[08])) > 0 , SC5->&(aObra[08]),"")) // UF para Obra
        cCodPaisOb := AllTrim(If(!Empty(aObra[09]) .And. SC5->(FieldPos(aObra[09])) > 0 , SC5->&(aObra[09]),"")) // Codigo do Pais para Obra
        cNomPaisOb := AllTrim(If(!Empty(aObra[10]) .And. SC5->(FieldPos(aObra[10])) > 0 , SC5->&(aObra[10]),"")) // Nome do Pais para Obra
        cNumArtOb  := AllTrim(If(!Empty(aObra[11]) .And. SC5->(FieldPos(aObra[11])) > 0 , SC5->&(aObra[11]),"")) // Numero Art para Obra
        cNumCeiOb  := AllTrim(If(!Empty(aObra[12]) .And. SC5->(FieldPos(aObra[12])) > 0 , SC5->&(aObra[12]),"")) // Numero CEI para Obra
        cNumProOb  := AllTrim(If(!Empty(aObra[13]) .And. SC5->(FieldPos(aObra[13])) > 0 , SC5->&(aObra[13]),"")) // Numero Projeto para Obra
        cNumMatOb  := AllTrim(If(!Empty(aObra[14]) .And. SC5->(FieldPos(aObra[14])) > 0 , SC5->&(aObra[14]),"")) // Numero de Mtricula para Obra
        cNumEncap  := AllTrim(If(!Empty(aObra[15]) .And. SC5->(FieldPos(aObra[15])) > 0 , SC5->&(aObra[15]),"")) // NumeroEncapsulamento

        // Criado para Campinas - SP - 3509502
        cInsMunObr  := AllTrim(If(!Empty(aObra[16]) .And. SC5->(FieldPos(aObra[16])) > 0 , SC5->&(aObra[16]),"")) // Inscrição Municipal Obra
        cCodSerObr  := AllTrim(If(!Empty(aObra[17]) .And. SC5->(FieldPos(aObra[17])) > 0 , SC5->&(aObra[17]),"")) // Codigo de Serviço da Obra
        If !Empty(aObra[18]) .And. SC5->(FieldPos(aObra[18])) > 0 
            cNumCIBOb   := AllTrim(SC5->&(aObra[18])) // Numero CIB para Obra
        endif
    EndIf
    ...
    If(!Empty(cNumCIBOb),aadd(aConstr,(cNumCIBOb)),aadd(aConstr,"") ) // Numero CIB para Obra
    ...
    If ValType(aImovel) <> "U"
        cLogradIm  := AllTrim(If(!Empty(aImovel[01]) .And. SC5->(FieldPos(aImovel[01])) > 0 , SC5->&(aImovel[01]),"")) //Logradouro para Imovel
        cCompleIm  := AllTrim(If(!Empty(aImovel[02]) .And. SC5->(FieldPos(aImovel[02])) > 0 , SC5->&(aImovel[02]),"")) //Complemento para Imovel
        cNumeroIm  := AllTrim(If(!Empty(aImovel[03]) .And. SC5->(FieldPos(aImovel[03])) > 0 , SC5->&(aImovel[03]),"")) // Numero para Imovel
        cBairroIm  := AllTrim(If(!Empty(aImovel[04]) .And. SC5->(FieldPos(aImovel[04])) > 0 , SC5->&(aImovel[04]),"")) // Bairro para Imovel
        cCepIm     := AllTrim(If(!Empty(aImovel[05]) .And. SC5->(FieldPos(aImovel[05])) > 0 , SC5->&(aImovel[05]),"")) // Cep para Imovel
        cCodMunIm  := AllTrim(If(!Empty(aImovel[06]) .And. SC5->(FieldPos(aImovel[06])) > 0 , SC5->&(aImovel[06]),"")) // Cod do Municipio para Imovel
        cNomMunIm  := AllTrim(If(!Empty(aImovel[07]) .And. SC5->(FieldPos(aImovel[07])) > 0 , SC5->&(aImovel[07]),"")) // Nome do municipio para Imovel
        cUfIm      := AllTrim(If(!Empty(aImovel[08]) .And. SC5->(FieldPos(aImovel[08])) > 0 , SC5->&(aImovel[08]),"")) // UF para Imovel
        cCodPaisIm := AllTrim(If(!Empty(aImovel[09]) .And. SC5->(FieldPos(aImovel[09])) > 0 , SC5->&(aImovel[09]),"")) // Codigo do Pais para Imovel
        cInscImob  := AllTrim(If(!Empty(aImovel[10]) .And. SC5->(FieldPos(aImovel[10])) > 0 , SC5->&(aImovel[10]),"")) // Inscrição imobiliária fiscal 
        cNumCIBIm  := AllTrim(If(!Empty(aImovel[11]) .And. SC5->(FieldPos(aImovel[11])) > 0 , SC5->&(aImovel[11]),"")) // Numero CIB para Imovel
    EndIf

    If(!Empty(cLogradIm),aadd(aImoveis,(cLogradIm)),aadd(aImoveis,"") )     //Logradouro para Imovel
    If(!Empty(cCompleIm),aadd(aImoveis,(cCompleIm)),aadd(aImoveis,"") )     //Complemento para Imovel
    If(!Empty(cNumeroIm),aadd(aImoveis,(cNumeroIm)),aadd(aImoveis,"") )     // Numero para Imovel
    If(!Empty(cBairroIm),aadd(aImoveis,(cBairroIm)),aadd(aImoveis,"") )     // Bairro para Imovel
    If(!Empty(cCepIm),aadd(aImoveis,(cCepIm)),aadd(aImoveis,"") )           // Cep para Imovel
    If(!Empty(cCodMunIm),aadd(aImoveis,(cCodMunIm)),aadd(aImoveis,"") )     // Cod do Municipio para Imovel
    If(!Empty(cNomMunIm),aadd(aImoveis,(cNomMunIm)),aadd(aImoveis,"") )     // Nome do municipio para Imovel
    If(!Empty(cUfIm),aadd(aImoveis,(cUfIm)),aadd(aImoveis,"") )             // UF para Imovel
    If(!Empty(cCodPaisIm),aadd(aImoveis,(cCodPaisIm)),aadd(aImoveis,"") )   // Codigo do Pais para Imovel
    If(!Empty(cInscImob),aadd(aImoveis,(cInscImob)),aadd(aImoveis,"") )     // Inscrição imobiliária fiscal 
    If(!Empty(cNumCIBIm),aadd(aImoveis,(cNumCIBIm)),aadd(aImoveis,"") )     // Numero CIB para Imovel
...
static function identNac( aNota, cAmbiente , cLocEmi)
    ...
    elseif ( !empty( cCamSC5 ) .and. SC5->( fieldpos( cCamSC5 ) ) > 0 )
        cString	+= '<dCompet>'+ subStr( dToS( dDateCom ), 1, 4 ) + "-" + subStr( dToS( dDateCom ), 5, 2 ) + "-" + subStr( Dtos( dDateCom ), 7, 2 ) + "</dCompet>"
    endif
...
static function substit( aNota )
	...
	Local cString	:= ""
    Local aArea     := SF3->(GetArea())
    Local cChave    := ""
    Local cCodcanc  := "99" // outros
    Local cMotivo   := "NOTA FISCAL EMITIDA COM ERRO, NECESSÁRIO SUBSTITUIÇÃO."

	
	If !empty( allTrim( aNota[7] ) + allTrim( aNota[8] ) )
        SF3->(DbSetOrder(5))

        if  SF3->(MsSeek(xFilial("SF3")+aNota[8]+aNota[7],.T.))
            cChave := alltrim(SF3->F3_CODNFE)

            cString	+= "<subst>"
            cString	+= "<chSubstda>" + cChave + "</chSubstda>"
            cString	+= "<cMotivo>" + cCodcanc + "</cMotivo>"
            cString	+= "<xMotivo>" + cMotivo + "</xMotivo>"		
            cString	+= "</subst>"		
        Endif 
	Endif
...
static function servicosNac( aProd, cNatOper, lNFeDesc, cDiscrNFSe,aRetSF3,aDest)	
    ...
    lExterior   := (type("aDest[9]") <> "U" .and. aDest[9] == "EX")
    cLocPrest   := prestacaoNac( cMunPrest, cDescMunP, aDest )
    
    If lExterior .and. !Empty(cLocPrest) .and. cLocPrest == "9999999"
        cString += '<cPaisPrestacao>'+ Tsspais(Alltrim(aDest[11])) +'</cPaisPrestacao>'
        cPaisPrest := "EX"
    Else
        cString += '<cLocPrestacao>'+ cLocPrest +'</cLocPrestacao>'
        cPaisPrest := "BR"
    EndIf
    ...
    If lUsaSF3
        cTrib := allTrim(aRetSF3[1][3])
    ElseIf !Empty(allTrim(aProd[1][32]))
        cTrib := allTrim(aProd[1][32])
    EndIf

    If !Empty(cTrib)
        cTribNac := SubStr(cTrib,1,6)
        cString +='<cTribNac>'+ cTribNac +'</cTribNac>'
        If LEN(cTrib) == 9
            cString +='<cTribMun>'+ SubStr(cTrib,7,3) +'</cTribMun>'
        EndIf
    EndIf
    ...
    cString += GerAtvEvt(cTribNac, cPaisPrest, cMunPrest)
...
static function valoresNac( aISSQN, aRetido, aTotal, aDest, cCodMun,aLeiTrp,lRecIrrf, aProd )
    ...
    if (!Empty( aDest[9] ) .AND. aDest[9] $ "EX" .and. aTotal[3] $ "2-3" .and. lConfTrIss  == .F.) .or. (aTotal[3] == "9" .and. lConfTrIss  == .T.) // Exportacao (ERP) dentro ou fora do municipio
        lGeraPaisR  := fPaisResu(cPaisPrest, cTribIssqn,cTribNac)
    ...
    ElseIf cLocEmi <>  cLocPrest .AND. cOPTSIMP == "2" .AND. ( Empty(cMVREGIESP) .OR. cMVREGIESP == "0" ) .AND. !cTribIssqn $ "2|3|4"// Não Optante pelo Simples Nacional
    ...
    If !Empty(aConstr[1]) .or. !Empty(aConstr[3]) .Or. !Empty(aConstr[4]) .or. !Empty(aConstr[15]) .or. !Empty(aConstr[17]) .or. !Empty(aConstr[19])
        ...
        Do Case // Esse CASE foi criado, pois a NFSe Nacional aceita apenas uma das informaçoes (<cObra>,<inscImobFisc>,<end>) na montagem do bloco de OBRA
            ...
            Case (Len(aConstr) > 03 .And. !Empty(aConstr[04]))
                cString += '<end>'  
                cString += If(Len(aConstr) > 07 .And. !Empty(aConstr[08]), '<CEP>'+ SubStr( StrTran(AllTrim(aConstr[08]), "-",""), 1, 8 ) +'</CEP>' , "")
...
/*/
//-----------------------------------------------------------------------
static function IbsCbs( aDest, cCodMun, aNota, cClieFor, cLoja, aEntrega, aRetSF3, aProd, aImoveis )
    Local cString	     := ""
    Local cErro          := ""
    Local cAviso         := ""
    Local cXmlIBSCBS     := ""
    ...
    If lUsaSF3
        cTribNac := allTrim(aRetSF3[1][3])
    ElseIf !Empty(allTrim(aProd[1][32]))
        cTribNac := allTrim(aProd[1][32])
    EndIf
    ...
    If !Empty(cDocumentItemId) .And. findClass('totvs.protheus.backoffice.tss.engine.xml.taxinformation') .And. DbSeek(xFilial("SD2")+aNota[2]+aNota[1]+cClieFor+cLoja)
        ...
        If type("oXmlIBSCBS") <> "U"
            cString += "<IBSCBS>"
                ...
                //Indicador da finalidade da emissão de NFS-e:
                //0 - NFS-e regular;
                If type("aDest[30]")<>"U" .and. aDest[30] == "F"
                ...
                //--------------------------------------------------------------------------------------------------------------
                //RT NFSe Nacional - Grupo de informações de operações relacionadas a bens imóveis, exceto obras. <imovel>
                //--------------------------------------------------------------------------------------------------------------
                If !Empty( aImoveis )
                    cString += ImovelNac( aImoveis )
                EndIf

                //--------------------------------------------------------------------------------------------------------------
                //RT NFSe Nacional - Grupo de informações relativas aos bens móveis objetos de locação <gLocBensMoveis>
                //--------------------------------------------------------------------------------------------------------------
                If ( lLocBac .And. StrTran( cTribNac, '.', '' ) $ "990401-00990401" .And. Len( aProd[1][44][01] ) > 0 ) //Integracao com módulo de Locações SIGALOC com BackOffice com Notas ou Pedido de venda ou Compras
                    cString += LocBensMov( aProd[1][44][01], aProd[1][44][02])
                EndIf
...
Return cString
/*/ {Protheus.doc} Static Function fPaisResu
	Responsável por retornar se deve ser gerado a tag cPaisResult.

	@author Gustavo Demate
	@since 20/03/2026
	@version 12.1.2510
    @type Function
    @param cPaisPrest Pais de prestação.
    @param cTribIssqn Tributação do ISSQN.
    @param cTribNac Código de Tributação Nacional.
    @return logical
/*/

Static Function fPaisResu(cPaisPrest, cTribIssqn,cTribNac)

    Local lRet      := .F.
    Local cIncidenc := ""
    Local oJTribNac := Nil

    oJTribNac := GetTribNac()

    If oJTribNac['cod_tribnac']:HasProperty(cTribNac)
        cIncidenc := oJTribNac['cod_tribnac'][cTribNac]
    Else
        cIncidenc := "EP"
    EndIf

    If cTribIssqn == "3" .and. cPaisPrest == "BR"
        If cIncidenc $ "EP|ET"
            lRet := .T.
        EndIf
    EndIf
...
/*/ {Protheus.doc} Static Function GetTribNac
	Responsável por buscar a incidência do cTribNac.
...
Static Function GetTribNac()

    Local oJsonRet  := JsonObject():New()
    Local oJsonTemp := JsonObject():New()
    ...
    oJsonRet['cod_tribnac'] := oJsonTemp

Return oJsonRet

//-----------------------------------------------------------------------
/*/{Protheus.doc} ParsAtvEvt
Realiza o parsing do parametro MV_ATVEVT e retorna array de mapeamento
tag XML x campo SC5 no formato {{cTag, cCampo}, ...}.
...
@return aMapAtv, Array bidimensional com pares {cTag, cCampo} ou array vazio
/*/
//-----------------------------------------------------------------------
Static Function ParsAtvEvt()

    Local cParAtv   := AllTrim(SuperGetMV("MV_ATVEVT", .F., ""))
    Local aMapAtv   := {}
    Local aPares    := {}
    Local aPartes   := {}
    Local nI        := 0
    ...
    aPares := StrTokArr(cParAtv, ";")

    For nI := 1 To Len(aPares)
        cPar := AllTrim(aPares[nI])

        If Empty(cPar)
            Loop
        EndIf

        aPartes := StrTokArr(cPar, "=")

        If Len(aPartes) <> 2 .Or. Empty(AllTrim(aPartes[1])) .Or. Empty(AllTrim(aPartes[2]))
            Loop
        EndIf

        aAdd(aMapAtv, {AllTrim(aPartes[1]), AllTrim(aPartes[2])})
    Next nI

Return aMapAtv

//-----------------------------------------------------------------------
/*/{Protheus.doc} GerAtvEvt
Orquestra a geracao condicional do grupo <atvEvento> no XML da NFS-e
Nacional. Gera o bloco somente quando cTribNac pertence ao item 12 da
lista de servicos ou eh 990101, o parametro MV_ATVEVT esta configurado
e os campos obrigatorios estao preenchidos no SC5.
...
@param cTribNac,   Caractere, Codigo de tributacao nacional (6 digitos)
@param cPaisPrest, Caractere, Pais da prestacao ("BR" ou "EX")
@param cMunPrest,  Caractere, Codigo IBGE do municipio de prestacao

@return cRetXml, Caractere, String XML do grupo <atvEvento> ou string vazia
/*/
//-----------------------------------------------------------------------
Static Function GerAtvEvt(cTribNac, cPaisPrest, cMunPrest)
    ...
    // 2. Verificar se cTribNac exige atvEvento
    If SubStr(cTribNac, 1, 2) <> "12" .And. cTribNac <> "990101"
        Return cRetXml
    EndIf

    // 3. Ler valores dos campos SC5
    For nI := 1 To Len(aMapAtv)
        cTag   := aMapAtv[nI][1]
        cCampo := aMapAtv[nI][2]

        nPos := SC5->(FieldPos(cCampo))

        If nPos == 0
            Loop
        EndIf

        xValor := SC5->(FieldGet(nPos))

        // Formatar datas para ISO YYYY-MM-DD
        If cTag == "dtIni" .Or. cTag == "dtFim"
            If ValType(xValor) == "D" .And. !Empty(xValor)
                cDtIso := SubStr(DtoS(xValor), 1, 4) + "-" + SubStr(DtoS(xValor), 5, 2) + "-" + SubStr(DtoS(xValor), 7, 2)
                xValor := cDtIso
        ...
        aAdd(aValores, {cTag, xValor})

        // Marcar campos obrigatorios e flags de controle
        If cTag == "xNome"
            lTemNome := !Empty(xValor)
        ElseIf cTag == "dtIni"
            lTemDtIni := !Empty(xValor)
        ElseIf cTag == "dtFim"
            lTemDtFim := !Empty(xValor)
        ElseIf cTag == "idAtvEvt"
            lTemIdAtv := !Empty(xValor)
        ElseIf cTag == "CEP"
            lTemCEP := !Empty(xValor)
        ElseIf cTag == "cEndPost" .Or. cTag == "xCidade" .Or. cTag == "xEstProvReg"
            If !Empty(xValor)
                lTemEndExt := .T.
            EndIf
        EndIf
    ...
    // 4. Verificar campos obrigatorios
    If !lTemNome .Or. !lTemDtIni .Or. !lTemDtFim
        Return cRetXml
    EndIf
    ...
    // 5.1 Tags diretas do atvEvento: xNome, dtIni, dtFim
    For nI := 1 To Len(aValores)
        cTag   := aValores[nI][1]
        xValor := aValores[nI][2]

        If Empty(xValor)
            Loop
        EndIf

        If cTag == "xNome" .Or. cTag == "dtIni" .Or. cTag == "dtFim"
            cRetXml += "<" + cTag + ">" + xValor + "</" + cTag + ">"
        EndIf
    Next nI

    // 5.2 choice: idAtvEvt OU end (TCEnderecoSimples)
    If lTemIdAtv
        // Etapa 1: Quando informado idAtvEvt, desconsiderar o grupo end e suas tags filhas
        For nI := 1 To Len(aValores)
            If aValores[nI][1] == "idAtvEvt" .And. !Empty(aValores[nI][2])
                cRetXml += "<idAtvEvt>" + aValores[nI][2] + "</idAtvEvt>"
            EndIf
        Next nI
    Else
        // Montar grupo <end> (TCEnderecoSimples)
        cRetXml += "<end>"

        // choice interno: CEP OU endExt
        If lTemCEP
            // Etapa 2: Quando informado CEP, ignorar endExt
            For nI := 1 To Len(aValores)
                If aValores[nI][1] == "CEP" .And. !Empty(aValores[nI][2])
                    cRetXml += "<CEP>" + aValores[nI][2] + "</CEP>"
                EndIf
            Next nI
        ElseIf lTemEndExt
            // Etapa 3: Sem CEP, com endExt (TCEnderExtSimples): cEndPost, xCidade, xEstProvReg
            cRetXml += "<endExt>"
            For nI := 1 To Len(aValores)
                cTag   := aValores[nI][1]
                xValor := aValores[nI][2]
                If (cTag == "cEndPost" .Or. cTag == "xCidade" .Or. cTag == "xEstProvReg") .And. !Empty(xValor)
                    cRetXml += "<" + cTag + ">" + xValor + "</" + cTag + ">"
                EndIf
            Next nI
            cRetXml += "</endExt>"
        ...
        // Tags comuns do TCEnderecoSimples: xLgr, nro, xCpl, xBairro
        For nI := 1 To Len(aValores)
            cTag   := aValores[nI][1]
            xValor := aValores[nI][2]
            If (cTag == "xLgr" .Or. cTag == "nro" .Or. cTag == "xCpl" .Or. cTag == "xBairro") .And. !Empty(xValor)
                cRetXml += "<" + cTag + ">" + xValor + "</" + cTag + ">"
            EndIf
        Next nI
...
//-----------------------------------------------------------------------
/*/{Protheus.doc}  Method LocBensMov
	Método geração do Grupo de informações relativas aos bens móveis objetos de locação <gLocBensMoveis>
		
	@param cPedidoSd2 Numero do pedido de venda (SD2) para localizar os itens relacionados a bens móveis locados.
    @param cItemSd2 Numero do item do pedido de venda (SD2) para localizar os itens relacionados a bens móveis locados.
	@return cString String com XML da tag <cNCMBemMovel>, <xNCMBemMovel>, <qtdNCMBemMovel>

	@author Frank Zwarg Fuga / Felipe Duarte Luna
	@since 11.06.2026
	@version 12.1.2510
/*///-----------------------------------------------------------------------
Static Function LocBensMov( cPedidoSd2, cItemSd2)
	Local cXmlLoc   := ""  // Retorno do xml alterado
    ...
    */
    ...
    If len( aResult ) > 0 // Se for um movimento do Rental faremos a montagem do xml
        For nPos := 1 to len( aResult )
            cResult := "<gLocBensMoveis>"
                // Código da Nomenclatura Comum do Mercosul (NCM) do bem móvel objeto da locação
                cResult += "<cNCMBemMovel>" + AllTrim( SubStr( aResult[nPos,3],1,8 ) ) + "</cNCMBemMovel>"
                ...
                cResult += "<xNCMBemMovel>" + cContem + "</xNCMBemMovel>"
                
                // Quantidade de Bem Móvel objeto de locação - Máscara em variável (transformação complexa)
                cContem := aResult[nPos,2]
                cResult += "<qtdNCMBemMovel>" + AllTrim( ConvType( cContem, 15, 2 ) ) + "</qtdNCMBemMovel>"
            cResult += "</gLocBensMoveis>"
        Next
    EndIf
...
//-------------------------------------------------------------------
/*/{Protheus.doc} ImovelNac
Função para montar a tag de construção civil do XML de envio de NFS-e ao TSS.
Gera o grupo <imovel> com identificação por cCIB ou endereço (<end>), conforme regras da Nota Técnica Nº 005 - Versão 1.1.

@type function
@author Felipe Duarte Luna
@since 16/06/2025

@param	aConstr		array		Array com mapeamento posicional MV_XMLOBRA
@return	cString		character	XML do grupo imovel ou string vazia em caso de erro
/*/
//-------------------------------------------------------------------
Static Function ImovelNac( aImoveis )
	...
	//---------------------------------------------------------------
	// Extrair dados do array aImoveis nas posições do parametro MV_XMLOBRA para que seja possível o usuário criar campos adicionais no pedido de venda (SC5), para dados do endereço da obra para que essas informações sejam transmitidas na NFS-e.
	//---------------------------------------------------------------
	If Len(aImoveis) >= 01
		cLogradour := AllTrim(aImoveis[01])
	EndIf
	If Len(aImoveis) >= 02
		cComplemen := AllTrim(aImoveis[02])
	EndIf
	If Len(aImoveis) >= 03
		cNumero := AllTrim(aImoveis[03])
	EndIf
	If Len(aImoveis) >= 04
		cBairro := AllTrim(aImoveis[04])
	EndIf
	If Len(aImoveis) >= 05
		cCep := StrTran(AllTrim(aImoveis[05]), "-","")
	EndIf
	If Len(aImoveis) >= 07
		cCidade := AllTrim(aImoveis[07])
	EndIf
	If Len(aImoveis) >= 08
		cEstProv := AllTrim(aImoveis[08])
	EndIf
    If ( Len(aImoveis) >= 09 .And. !Empty(AllTrim(aImoveis[09])) )
        cPais := AllTrim(aImoveis[09])
	EndIf
	If Len(aImoveis) >= 10
		cInscImob := AllTrim(aImoveis[10])
	EndIf
    If Len(aImoveis) >= 11
		cCCIB := AllTrim(aImoveis[11])
	EndIf

    //---------------------------------------------------------------
	// Se nenhum conteudo extraido do array tiver preenchimento, retorna vazio sem gerar o grupo <imovel>
	//---------------------------------------------------------------
    If !Empty(cCCIB) .Or. !Empty(cInscImob) .Or. !Empty(cLogradour) .Or. !Empty(cComplemen) .Or. !Empty(cNumero) .Or. !Empty(cBairro) .Or. !Empty(cCep) .Or. !Empty(cCidade) .Or. !Empty(cEstProv)

        //---------------------------------------------------------------
        // Gerar inscImobFisc (independente da escolha cCIB/end)
        //---------------------------------------------------------------
        If !Empty(cInscImob)
            cString := '<inscImobFisc>' + SubStr( cInscImob, 1, 30 ) + '</inscImobFisc>'
        EndIf

        //---------------------------------------------------------------
        // Escolha exclusiva: cCIB ou end
        //---------------------------------------------------------------
        If !Empty(cCCIB)

            cString += '<cCIB>' + SubStr( cCCIB, 1, 8 ) + '</cCIB>'
        Else
            //-----------
            //- end
            //-----------
            ...
            If cPais $ "105|1058|01058"
                //-------------------------------------------------------
                // Endereço Brasil: gerar CEP, omitir endExt
                //-------------------------------------------------------
                If !Empty(cCep)
                    cString += '<CEP>' + SubStr( cCep, 1, 8 ) + '</CEP>'
                EndIf    
            Else
                //-------------------------------------------------------
                // Endereço Exterior: gerar endExt, omitir CEP
                //-------------------------------------------------------
                cEndPost := AllTrim(cCep) // Reutiliza posição [08] como código postal exterior
                If !Empty(cEndPost) .Or. !Empty(cCidade) .Or. !Empty(cEstProv)
                    cString += '<endExt>'
                    If !Empty(cEndPost)
                        cString += '<cEndPost>' + SubStr( cEndPost, 1, 11 ) + '</cEndPost>'
                    EndIf
                    If !Empty(cCidade)   
                        cString += '<xCidade>' + SubStr( cCidade, 1, 60 ) + '</xCidade>'
                    EndIf
                    If !Empty(cEstProv)
                        cString += '<xEstProvReg>' + SubStr( cEstProv, 1, 60 ) + '</xEstProvReg>'
                    EndIf
                    cString += '</endExt>'
                EndIf
            EndIf
            If !Empty(cLogradour)
                cString += '<xLgr>' + SubStr( cLogradour, 1, 255 ) + '</xLgr>'
            EndIf
            If !Empty(cNumero)
                cString += '<nro>' + SubStr( cNumero, 1, 60 ) + '</nro>'
            EndIf
            If !Empty(cComplemen)
                cString += '<xCpl>' + SubStr( cComplemen, 1, 156 ) + '</xCpl>'
            EndIf
            If !Empty(cBairro)
                cString += '<xBairro>' + SubStr( cBairro, 1, 60 ) + '</xBairro>'
            EndIf
        ...
        //---------------------------------------------------------------
	    // Envolver com tag <imovel>
        //---------------------------------------------------------------
        cString := '<imovel>' + cString + '</imovel>'
    EndIf    
