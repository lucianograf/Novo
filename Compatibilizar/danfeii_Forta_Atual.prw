...
/*Compatibilizado em 28/07/206 - Luciano - Lauschner Consulting*/
#DEFINE IMP_SPOOL 2
...
// Customização Grupo Forta - Função para impressão externa 
...
User Function MLDNFPROC(nType,cPasta,cSerie,cNota,dInEmissao)
	...
	//Gera o XML da Nota
	cArquivo := AllTrim(cNota) + "_" + dToS(MSDate()) + "_" + StrTran(Time(), ":", "") //+".pdf"
	...
	U_DANFEProc(@oDanfe, @lEnd, cIDEnt, /*cVal1*/, /*cVal2*/, /*@lExistNFe*/, /*lIsLoja*/, 0/*nTipo*/)
	//&cProg.(@oDanfe, 		,cIDEnt, Nil, Nil, @lExistNFe, lIsLoja)
	//oDanfe:Preview()//Visualiza antes de imprimir
	oDanfe:Print()
	...
	//Ferase(cDir+cDest)
	__CopyFIle(GetTempPath() + "totvsprinter\" + cArquivo , cPasta  + cArquivo )
...
User Function DANFEProc(	oDanfe	, lEnd		, cIdEnt	, cVal1,;
	...
	Local iP 		:= 0 // Customização Grupo Forta
	Default lEnd		:= .F.
	Default lIsLoja		:= .F.
	Default nTipo		:= 0
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					...
					If ValAtrib("oImposto:_Imposto:_ICMS")<>"U"
						...
						For nY := 1 To nLenSit
							...
							If ValAtrib("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2])<>"U" .Or. ValAtrib("oImposto:_Imposto:_ICMS:_ICMSST")<>"U"
								...
								EndIf
							EndIf
							// Customização Grupo Forta - 23/03/2026 - Adiciona texto de valores do st em cada item
							//If Type("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_pMVAST") <> "U"
							//	cTxtAuxImp	+= Padr("MVA ST: " + Alltrim(Transform(Val(&("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_pMVAST:TEXT")),"@E 999.99%")),nMaxDes)
							//Endif
							//If Type("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_pRedBCST") <> "U"
							//	cTxtAuxImp	+= Padr("% Red.ST: " + Alltrim(Transform(Val(&("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_pRedBCST:TEXT")),"@E 999.99%")),nMaxDes)
							//Endif
							//If Type("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_vBCST") <> "U"
							//	cTxtAuxImp	+= Padr("Base ST: R$ " + Alltrim(Transform(Val(&("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_vBCST:TEXT")),"@E 999,999,999.99")),nMaxDes)
							//Endif
							//If Type("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_pICMSST") <> "U"
							//	cTxtAuxImp	+= Padr("ICMS ST: " + Alltrim(Transform(Val(&("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_pICMSST:TEXT")),"@E 999.99%")),nMaxDes)
							//Endif
							If Type("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_vICMSST") <> "U"
								cTxtAuxImp	+= Padr("ICMS ST: R$ " + Alltrim(Transform(Val(&("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_vICMSST:TEXT")),"@E 999,999,999.99")),nMaxDes)
							Endif
						Next nY
	...
	// 23/03/2026 - Customização Grupo Forta - Adiciona informação do ST por item 
	If !Empty(cTxtAuxImp)
		cAux 	:= Space(nMaxDes) + cTxtAuxImp
	
		While !Empty(cAux)
			aadd(aItens,{;
					"",;
					{SubStr(cAux,1,nMaxDes), .T.},;
					"",;
					"",;
					"",;
					"",;
		...
		EndDo
	Endif 
	// Final customização Grupo Forta
